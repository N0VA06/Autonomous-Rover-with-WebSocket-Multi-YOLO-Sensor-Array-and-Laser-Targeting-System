...
class RoverNavigation:
    def __init__(self, camera_id=0, resolution=(640, 480), goal_coordinates=(100, 100)):
        ...
        self.runtime_log = []  # For logging simulation status with timestamp
...
def save_performance_metrics(self):
    ...
    elapsed_time = time.time() - self.start_time
    fps = self.frame_count / elapsed_time if elapsed_time > 0 else 0
    detection_ms = [dt * 1000 for dt in self.detection_times]
    planning_ms = [pt * 1000 for pt in self.planning_times]

    print(f"Final FPS: {fps:.2f}")
    print(f"Avg Detection Time: {np.mean(detection_ms):.2f} ms")
    print(f"Avg Planning Time: {np.mean(planning_ms):.2f} ms")
    print(f"Total Replans: {self.replanning_count}")

    # Save runtime log to CSV
    try:
        # Save the log to CSV if not already saved
        df = pd.DataFrame(self.runtime_log)
        df.to_csv("runtime_log.csv", index=False)
        print("Runtime log saved to 'runtime_log.csv'")
    except Exception as e:
        print(f"Failed to save runtime log: {e}")
        return

    # Load the saved CSV file for plotting
    try:
        df = pd.read_csv("runtime_log.csv")
    except Exception as e:
        print(f"Failed to load runtime log: {e}")
        return

    # Drop incomplete rows
    df.dropna(subset=["detection_time", "planning_time"], inplace=True)
    if df.empty:
        print("No valid data to plot after cleaning.")
        return

    # Convert to ms
    df["detection_time_ms"] = df["detection_time"] * 1000
    df["planning_time_ms"] = df["planning_time"] * 1000

    # Plot graphs
    plt.figure(figsize=(12, 6))

    # Plot Processing Times Over Time
    plt.subplot(2, 1, 1)
    plt.plot(df["timestamp"], df["detection_time_ms"], label="Detection Time (ms)")
    plt.plot(df["timestamp"], df["planning_time_ms"], label="Planning Time (ms)")
    plt.title("Processing Times Over Time")
    plt.xlabel("Time (s)")
    plt.ylabel("Time (ms)")
    plt.legend()
    ...
    # Plot FPS
    plt.subplot(2, 1, 2)
    plt.plot(df["timestamp"], df["fps"], 'g--', label="FPS")
    plt.xlabel("Time (s)")
    plt.ylabel("FPS")
    plt.legend()
    ...
    print("Saved performance plot as 'detailed_performance.png'")
    plt.show()
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                # Distance to goal (in world meters)
                goal_world_x, goal_world_y = self.world_goal_position
                rover_world_x = self.current_position[0] * self.grid_to_world_scale_x
                rover_world_y = self.current_position[1] * self.grid_to_world_scale_y
                dist_to_goal = np.sqrt((rover_world_x - goal_world_x)**2 + (rover_world_y - goal_world_y)**2)

                # Occupancy %
                occupancy_percent = np.sum(self.occupancy_grid > 0.5) / (self.grid_size[0] * self.grid_size[1]) * 100

                # Average traversability score
                traversability_score = np.mean(self.traversability_grid)

                # Object counts
                static_count = len([obj for obj in classified_objects if obj['type'] == 'static'])
                dynamic_count = len([obj for obj in classified_objects if obj['type'] == 'dynamic'])

                self.runtime_log.append({"timestamp": time.time() - self.start_time,"frame": self.frame_count,"fps": self.frame_count / (time.time() - self.start_time + 1e-5),"command": command,"position": self.current_position,"orientation": self.current_orientation,"left_velocity": left_vel,"right_velocity": right_vel,"detection_time": self.detection_times[-1] if self.detection_times else None,"planning_time": self.planning_times[-1] if self.planning_times else None,"path_length": len(self.path) if self.path else 0,"distance_to_goal": dist_to_goal,"occupancy_percent": occupancy_percent,"traversability_score": traversability_score,"static_objects": static_count,"dynamic_objects": dynamic_count})
...
class SimulatedRoverNavigation(RoverNavigation):
    ...
    def __init__(self, resolution=(640, 480), goal_coordinates=(100, 100)):
        ...
        world_width, world_height = 12.0, 12.0  # in meters
        self.grid_to_world_scale_x = world_width / self.grid_size[0]
        self.grid_to_world_scale_y = world_height / self.grid_size[1]
